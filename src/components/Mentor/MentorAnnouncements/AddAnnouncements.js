...
const AddAnnouncement = () => {
  ...
  const handleSubmit = async (e, status = "published") => {
    ...
    try {
      ...
      formDataObj.append("author_type", "mentor");
      formDataObj.append("author_id", author_id);
      ...
      } else if (existingFile?.url && !isEditMode) {
        // If we want to keep the existing file when creating a new one (not usual)
        // In most cases for POST, if no new file is selected, we don't send attachment
      }
