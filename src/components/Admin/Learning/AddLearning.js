...
const handleSubmit = async (e) => {
  ...
  try {
      ...
      sessions: sessions.map((session) => {
        const sessionObj = {
          ...
          videos: session.videos.map((video) => {
            ...
            // Only include ID if it's an existing DB ID (not generated by Date.now())
            if (isEditMode && video.id && String(video.id).length < 12) {
        ...
        // Only include ID if it's an existing DB ID (not generated by Date.now())
        if (isEditMode && session.id && String(session.id).length < 12) {
    ...
    const url = isEditMode
      ? `${baseUrlClean}/api/mentor/learning-modules/${id}/`
      : `${baseUrlClean}/api/mentor/learning-modules/`;
                          ...
                          <div className="row">
                            ...
                            <div className="col-md-6 mb-3">
                              ...
                              <textarea
                                ...
                                onChange={(e) => updateVideo(sessionIndex, videoIndex, 'description', e.target.value)}
                                rows="1"
